...
| `rails` → `hatchbox` | Rails | Deploy a Rails app on Hatchbox.io via the `hatchbox` CLI — app create, env vars, RAILS_MASTER_KEY, Postgres provisioning and attachment, custom domains with Caddy/Let's Encrypt SSL, GitHub auto-deploy |
| `rails` → `new-app-checklist` | Rails | Production readiness checklist for a new Rails app — routes to the other Rails skills in dependency order (conventions → key backup → auth → Hatchbox → Cloudflare → Postmark → SEO → monitoring) |

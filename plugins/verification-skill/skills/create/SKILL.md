---
name: create
description: "Generate a project-local verification skill that drives your app the way a user does — any language, framework, or platform. Use for /verification-skill:create, \"make a control skill for this repo\", or when a project has no scripted way to prove UI/CLI/service behavior."
...
Make the generated skill **model-invocable**: do NOT set `disable-model-invocation` on it, and write its `description` as trigger conditions (the surfaces and phrases that should pull it in — "verify the app", "prove <feature> works", "drive the UI/CLI", after a change to a user-facing surface) so an agent auto-activates it mid-task, not only on an explicit slash command. Its sections:
...
Point the user at `/verification-skill:maintain` for keeping the map honest as the app changes. Suggest a cadence only if they ask.
